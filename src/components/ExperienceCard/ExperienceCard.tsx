  ...
  return (
    <div className="organisation-info__content">
      <div className="organisation-info__header">
        ...
        <img
          className="organisation-info__image"
          src={defaultImageUrl}
          alt={experience.organisation.name}
        />
      </div>

      <div className="organisation-info__section">
        <h3
          className="organisation-info__toggle-header"
          onClick={() => setShowAbout(!showAbout)}
        >
          <ExpandMoreIcon
            className={`organisation-info__toggle-icon ${
              showAbout ? "expanded" : ""
            }`}
          />
          About
        ...
        {showAbout && (
          <p className="organisation-info__overview">
            {experience.organisation.overview ?? `No description available`}
          </p>
        )}
      </div>
        ...
        {showAchievements && (
          <ul className="organisation-info__achievements-list">
            {achievementItems.map((item, index) => (
              <li className="organisation-info__achievement-item" key={index}>
                {item}
              </li>
            ))}
          </ul>
